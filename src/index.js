...
async function handleHTMLRequest(request, env, pathname) {
...
async function getRankings(env, pollType) {
    ...
    const stmt = env.DB.prepare('SELECT * FROM rankings WHERE poll_type = ? ORDER BY week_date DESC, rank ASC LIMIT 25');
...
async function getStandings(env, conference) {
    ...
    const stmt = env.DB.prepare('SELECT * FROM standings WHERE conference = ? ORDER BY week_date DESC, conference_wins DESC, overall_wins DESC');
...
async function updateRankingsAndStandings(env) {
    ...
    try {
        ...
        if (bigTenStandings && bigTenStandings.length > 0) {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `);
                await stmt.bind('Big Ten', team.school, team.confWins || 0, team.confLosses || 0, team.overallWins || 0, team.overallLosses || 0, team.pointsFor || 0, team.pointsAgainst || 0, team.homeRecord || '0-0', team.awayRecord || '0-0', team.streak || '', currentDate, new Date().toISOString()).run();
            }
        }
        // Clear caches
