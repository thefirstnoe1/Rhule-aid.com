export async function handleBettingRequest(request, env) {
...
async function fetchRealBettingLine(request, team1, team2) {
    try {
        ...
        console.log('Betting API debug:', {
            ...
            gamesWithOdds: cfbData.data.filter((g) => g.odds).length,
            firstGameWithOdds: cfbData.data.find((g) => g.odds) ? {
                home: cfbData.data.find((g) => g.odds).homeTeam.displayName,
                away: cfbData.data.find((g) => g.odds).awayTeam.displayName,
                homeAbbr: cfbData.data.find((g) => g.odds).homeTeam.abbreviation,
                awayAbbr: cfbData.data.find((g) => g.odds).awayTeam.abbreviation
            } : null
        });
        // Find the game between these two teams
        const game = cfbData.data.find((g) => {
            if (!g.odds)
                return false; // Skip games without odds
        ...
        if (!game || !game.odds) {
            ...
            console.log('Debug info:', {
                ...
                totalGames: cfbData.data.length,
                gamesWithOdds: cfbData.data.filter((g) => g.odds).length,
                availableMatchups: cfbData.data.filter((g) => g.odds).slice(0, 3).map((g) => ({
