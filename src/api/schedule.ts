...
export async function handleScheduleRequest(request: Request, env: any): Promise<Response> {
  ...
  try {
    ...
    if ((cfbdResponse as any).error) {
      throw new Error(`CFBD API error: ${JSON.stringify((cfbdResponse as any).error)}`);
    }

    const cfbdGames = Array.isArray((cfbdResponse as any).data) ? (cfbdResponse as any).data : [];
    const cfbdMedia = Array.isArray((mediaResponse as any).data) ? (mediaResponse as any).data : [];
...
async function fetchHuskerScheduleOverrides(season: number): Promise<Map<string, HuskerScheduleOverride>> {
  try {
...
function parseCFBDSchedule(cfbdGames: any[], mediaByGame: Map<string, string>): ScheduleGame[] {
  return cfbdGames
...
function parseCFBDGame(game: any, mediaByGame: Map<string, string>): ScheduleGame | null {
...
function createMediaLookup(media: any[]): Map<string, string> {
...
function getTVNetwork(game: any, mediaByGame: Map<string, string>): string {
