export async function handleLogoRequest(request: Request, _env: any): Promise<Response> {
...
function normalizeTeamName(teamName: string): string {
  ...
  if (teamName.toLowerCase().includes('michigan state')) return 'michigan state';
  if (teamName.toLowerCase().includes('penn state')) return 'penn state';
  if (teamName.toLowerCase().includes('ohio state')) return 'ohio state';
  if (teamName.toLowerCase().includes('iowa state')) return 'iowa state';
  if (teamName.toLowerCase().includes('kansas state')) return 'kansas state';
  if (teamName.toLowerCase().includes('oklahoma state')) return 'oklahoma state';
  if (teamName.toLowerCase().includes('arizona state')) return 'arizona state';
  if (teamName.toLowerCase().includes('oregon state')) return 'oregon state';
  if (teamName.toLowerCase().includes('washington state')) return 'washington state';
  if (teamName.toLowerCase().includes('mississippi state')) return 'mississippi state';
  if (teamName.toLowerCase().includes('florida state')) return 'florida state';
  if (teamName.toLowerCase().includes('north carolina state') || teamName.toLowerCase().includes('nc state')) return 'nc state';
  
  // For other teams, remove trailing "state"
  return normalized.replace(/\s+state$/i, '').trim();
}

function getTeamLogoMap(): Record<string, string> {
  return {
    // Big Ten Teams
