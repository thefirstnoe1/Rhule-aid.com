...
function parseESPNSchedule(espnData) {
    const games = [];
    ...
    for (const event of espnData.events) {
        ...
        const nebraskaCompetitor = competitors.find((c) => c.team.id === '158');
        const opponentCompetitor = competitors.find((c) => c.team.id !== '158');
        if (!nebraskaCompetitor || !opponentCompetitor)
            continue;
