// Rankings API for Cloudflare Worker
...
export async function handleRankingsRequest(request: Request, env: any, pollType: string): Promise<Response> {
  ...
  try {
    ...
    let cachedData: CachedRankingsData | null = null;
    ...
    if (env.RANKINGS_CACHE && rankings.length > 0) {
      try {
        const cacheData: CachedRankingsData = {
          teams: rankings,
          timestamp: Date.now(),
          source: 'mock-data'
        };
...
function getMockRankings(_pollType: string): RankingTeam[] {
