export async function handleSoccerRequest(request, env) {
    const headers = {
    ...
    try {
        ...
        const huskerWomens = (huskerData?.data || [])
            .filter((game) => game.datetime) // Filter first to ensure we have datetime
            .map((game) => ({
            id: game.id.toString(),
